        ...
        <FormItem label="商品" v-if="$route.query.id">
          <Table class="goods-table" :columns="liveColumns" :data="liveData">
            ...
            <template slot-scope="{ row }" class="price" slot="price">
              <div>
                <div v-if="row.priceType == 1">{{row.price | unitPrice('Kz')}}</div>
                <div v-if="row.priceType == 2">{{row.price | unitPrice('Kz')}}至{{row.price2 | unitPrice('Kz')}}</div>
                <div v-if="row.priceType == 3">{{row.price | unitPrice('Kz')}}<span class="original-price">{{row.price2 | unitPrice('Kz')}}</span></div>
              </div>
            </template>
