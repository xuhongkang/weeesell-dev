    ...
    <swiper :options="swiperOption" ref="mySwiper">
      <swiper-slide v-for="(item,index) in goodsList" :key="index">
        <div class="content hover-pointer"  @click.stop="goToSeckill">
          ...
          <div>
            <span>{{ item.price | unitPrice('Kz') }}</span>
            <span>{{ item.originalPrice | unitPrice('Kz') }}</span>
          </div>
        </div>
      </swiper-slide>
...
export default {
  ...
  methods: {
    ...
    countDown (currIndex) { // 倒计时
      // 0点时间戳
      var now = new Date();
      let zeroTime = new Date(now.toLocaleDateString()).getTime();
      let currTime = now.getTime()
      let actTime = 0;
      let nowHour = now.getHours(); // 当前小时数
