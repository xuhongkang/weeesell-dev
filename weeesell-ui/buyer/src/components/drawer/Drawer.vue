      ...
      <ul>
        <li v-for="(goods,goodsIndex) in cartList" :key="goodsIndex">
          ...
          <div>
            ...
            <p class="price">{{goods.goodsSku.price | unitPrice('Kz')}}<span>&nbsp; x{{goods.num}}</span></p>
          </div>
    ...
    <div v-else-if="title === '我的订单'" class="order-con">
      <ul>
        <li v-for="(order,orderIndex) in orderList" :key="orderIndex">
          ...
          <div class="order-handle"><span>{{ order.flowPrice | unitPrice("Kz") }}</span><span class="hover-color" @click="linkTo(`home/OrderDetail?sn=${order.sn}`)">查看订单</span></div>
        </li>
      </ul>
    ...
    <div v-else-if="title === '优惠券'" class="coupon-con">
      <ul class="coupon-list">
          <li v-for="(coupon, index) in couponList" class="coupon-item" :key="index">
            <div class="c-left">
              <div>
                <span v-if="coupon.couponType === 'PRICE'" class="fontsize_12 global_color">Kz<span class="price">{{coupon.price | unitPrice}}</span></span>
    ...
    <div v-else-if="title === '我的足迹'" class="tracks-con">
      <ul>
        <li v-for="(track,trackIndex) in tracksList" :key="trackIndex">
          ...
          <p class="global_color">{{track.price | unitPrice('Kz')}}</p>
        </li>
      </ul>
    ...
    <div v-else-if="title === '我的收藏'" class="collect-con">
      <ul>
        <li v-for="(collect,collectIndex) in collectList" :key="collectIndex">
          ...
          <p class="global_color">{{collect.price | unitPrice('Kz')}}</p>
        </li>
      </ul>
