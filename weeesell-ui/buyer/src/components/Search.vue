  ...
  <div class="navbar" :class="{'small-search-box':useClass == 'fixed-show'}">
    <div class="container width_1200_auto flex flex-a-c">
      <img
        ...
        @click="$router.push('/')"
      />
      <div :class="{'small-search-box':useClass == 'fixed-show'}" class="search-box" style="margin-left: 20%;">
        <i-input
          v-model="searchData"
          size="large"
          class="search "
          placeholder="Introduza o produto que pretende procurar"
...
export default {
  ...
  created() {
    ...
    if (!this.hover) { // 首页顶部固定搜索栏不调用热词接口
      ...
      var now = new Date();
      const time = now.getTime() - 5 * 60 * 1000
      if (!reloadTime) {
        ...
        storage.setItem('hotWordsReloadTime', now.getTime())
      }
    }
  }
};
