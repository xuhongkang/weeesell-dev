  ...
  <div class="pay-order">
    ...
    <div class="content width_1200">
            ...
            <div class="goods-list">
              <div class="goods-item" v-for="(goods, goodsIndex) in shop.checkedSkuList" :key="goodsIndex">
                ...
                <span class="goods-price">{{
                    goods.purchasePrice | unitPrice("Kz")
                }}</span>
                <span>x{{ goods.num }}</span>
                <span>{{ goods.goodsSku.quantity > 0 ? "有货" : "无货" }}</span>
                <span class="goods-price">{{
                    goods.subTotal | unitPrice("Kz")
                }}</span>
              </div>
            </div>
      ...
      <div class="invoice">
        ...
        <ul v-else class="coupon-list">
          <li v-for="(item, index) in couponList" class="coupon-item" :key="index">
            <div class="c-left">
              <div>
                <span v-if="item.couponType === 'PRICE'" class="fontsize_12 global_color">Kz<span class="price">{{
      ...
      <div class="order-price">
        <div>
          <span>{{ totalNum }}件商品，总商品金额：</span><span>{{ priceDetailDTO.goodsPrice | unitPrice("Kz") }}</span>
        </div>
        <div v-if="priceDetailDTO.freightPrice > 0">
          <span>运费：</span><span>{{ priceDetailDTO.freightPrice | unitPrice("Kz") }}</span>
        </div>
        <div v-if="priceDetailDTO.discountPrice > 0">
          <span>优惠金额：</span><span>-{{ priceDetailDTO.discountPrice | unitPrice("Kz") }}</span>
        </div>
        <div v-if="priceDetailDTO.couponPrice > 0">
          <span>优惠券金额：</span><span>-{{ priceDetailDTO.couponPrice | unitPrice("Kz") }}</span>
        </div>
        ...
        <div v-else>
          <span>应付金额：</span><span class="actrual-price">{{
              priceDetailDTO.flowPrice | unitPrice("Kz")
          }}</span>
        </div>
      </div>
    </div>
