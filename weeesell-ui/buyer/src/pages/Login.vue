<template>
  <div class="login" @keyup.enter="handleSubmit('formInline')">
    ...
    <div class="top-content" @click='$refs.verify.show = false'>
      <div class="logo-box">
        <img :src="$store.state.logoImg" @click="$router.push('/')" style="width: 85px;"/>
        <div><span>{{ $t('hydl') }}</span></div>
      </div>
    </div>
    <!-- 登录主体 包含轮播图 登录模块 -->
...
<!--            <span>{{ type ? '账号登录' : '验证码登录' }}</span>-->
            <span>{{ type ? $t('zhdl') : $t('yzmdl') }}</span>
            <span @click="type = !type,scannerCodeLoginFLag=false">{{ type ? $t('yzmdl') : $t('zhdl') }}</span>
          </div>
          <!---->
          <div @click="scannerCodeLoginFLag=!scannerCodeLoginFLag">{{!scannerCodeLoginFLag ? $t('smdl') : $t('fh')}}</div>
        </div>
        <!--扫码登录-->
        <div v-show="scannerCodeLoginFLag">
          <div class="qr-container">
            <div class='qr-shadow flex' v-show="qrCodeStatus == 'fail'">
              ...
              <Button size='small' @click="createPCLoginSession">{{$t('sxewm')}}</Button>
            </div>
            <vue-qr
              :text="qrCode"
              :margin="0"
        ...
        <div>
          ...
          <Form ref="formInline" :model="formData" :rules="ruleInline" v-show="type === true"
            ...
            <FormItem>
              <Button type="error" @click.stop="handleSubmit('formInline')" long>{{$t('login')}}</Button>
            </FormItem>
          </Form>
          <!-- 验证码登录 -->
          <Form ref="formSms" :model="formSms" :rules="ruleInline" v-show="type === false"
            ...
            <FormItem prop="mobile">
              <i-input type="text" v-model="formSms.mobile" clearable placeholder="phone">
                <Icon type="md-lock" slot="prepend"></Icon>
              </i-input>
            </FormItem>
            <FormItem prop="code">
              <i-input type="text" v-model="formSms.code" placeholder="Mobile verification code">
                <Icon type="ios-text-outline" style="font-weight: bold" slot="prepend"/>
                <Button slot="append" @click="sendCode">{{ $t('fsyzm') }}</Button>
              </i-input>
            </FormItem>
            <FormItem>
              <Button @click.stop="verifyBtnClick" long
                      :type="verifyStatus?'success':'default'">{{ verifyStatus ? $t('yztg') : $t('djwc') }}
              </Button>
            </FormItem>
            <FormItem>
              <Button type="error" @click="handleSubmit('formSms')" long>{{$t('login')}}</Button>
            </FormItem>
          </Form>
        </div>
          <div class="other">
...
<!--            </div>-->
            <div class="register">
              <span style="color:red" @click="$router.push('signUp')">{{$t('mzh')}}</span>
              <span @click="$router.push('forgetPassword')">{{$t('wjma')}}</span>
            </div>
          </div>
        </div>
      </div>
