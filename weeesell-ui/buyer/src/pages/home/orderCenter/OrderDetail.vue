  ...
  <div class="order-detail" v-if="order.order">
    ...
    <div class="goods">
      ...
      <table>
        ...
        <tbody>
          <tr v-for="(goods, goodsIndex) in order.orderItems" :key="goodsIndex">
            ...
            <td>{{ goods.id }}</td>
            <td>{{ goods.goodsPrice | unitPrice("Kz") }}</td>
            <td>{{ goods.num }}</td>
            <td>{{ (goods.goodsPrice * goods.num) | unitPrice("Kz") }}</td>
            <td>
              <Button
                v-if="
      ...
      <div class="order-price">
        ...
        <div>
          <span>商品总价：</span
          ><span>{{ order.order.goodsPrice | unitPrice("Kz") }}</span
          ><br />
        </div>
        <div>
          <span>运费：</span
          ><span>+{{ order.order.freightPrice | unitPrice("Kz") }}</span
          ><br />
        </div>
        <div v-if="order.order.priceDetailDTO.couponPrice">
          ...
          ><span
            >-{{
              order.order.priceDetailDTO.couponPrice || 0 | unitPrice("Kz")
            }}</span
          >
        </div>
        <div v-if="order.order.discountPrice">
          <span>活动优惠：</span
          ><span>-{{ order.order.discountPrice | unitPrice("Kz") }}</span>
        </div>
        <div>
          <span>应付金额：</span>
          <span class="actrual-price">{{
            order.order.flowPrice | unitPrice("Kz")
          }}</span>
        </div>
      </div>
    </div>
