      ...
      <div class="box">
        <div class="mb_20 account-price">
          ...
          <span class="fontsize_48 global_color"
            >Kz{{ result.canRebate | unitPrice }}</span
          >
          <span class="subTips">冻结金额：</span>
          <span class="">Kz{{ result.commissionFrozen | unitPrice }}</span>
          <span class="subTips">返利总金额：</span>
          <span class="">Kz{{ result.rebateTotal | unitPrice }}</span>
          <Button
            type="primary"
            size="small"
            class="ml_20"
        ...
        <TabPane label="已选商品" name="goodsChecked">
          <Table stripe :columns="goodsColumns" :data="goodsData.records">
            ...
            <template slot-scope="{ row }" slot="price">
              <span> Kz{{ row.price | unitPrice }}</span>
            </template>
            <template slot-scope="{ row }" slot="commission">
              <span> Kz{{ row.commission | unitPrice }}</span>
            </template>
