<template>
  <u-popup class="popup" v-model="addressFlag" :height="setup.height" :mode="setup.mode" :border-radius="setup.radius" @close="closeAddress()"   closeable>
    <view class="header-title">{{$t('xuaznedizhi')}}</view>
    ...
    <view class="view-box" v-else>
      <view class="noMore">
        <u-empty text="No Data" mode="address"></u-empty>
      </view>
    </view>
    ...
    <view class="btns">
      <view class="box-btn light" @click="getpicker">{{$t('qingxuanzedizhi')}}</view>
      <view class="box-btn" @click="closeAddress()">{{$t('qd')}}</view>
    </view>
    <m-city :provinceData="cityList" headTitle="Selecção da região" ref="cityPicker"  pickerSize="4"></m-city>
  </u-popup>
</template>
<script>
