  ...
  <div class="wrapper">
    <div class="box">
      <div class="block block-1">
        ...
        <p class="ptips">{{$t('shouyintai')}}</p>

        <p class="ptips">{{$t('shengyuzhifushijian')}}：
        ...
        <p class="ptips">
          {{$t('zhifujine')}}
          <span>Kz{{ cashierParams.price | unitPrice }}</span>
        </p>
      </div>
    </div>
    ...
    <div class="block-4" v-if="cashierParams.price > 0">
      ...
      <div class="payItem">{{$t('zhifufangshi')}}</div>
      <div class="payItem">Para finalizar o pagamento, contacte o nosso serviço de apoio ao cliente através: 
925 664 781 (Casa Sunny, Seg-Sáb, WhatsApp & Chamada)
925 230 333 (Casa Sunny, Seg-Sáb, Chamada)
940 962 441 (Garciano, Seg-Dom, WhatsApp & Chamada)</div>
...
<!--            <div v-if="item == 'WECHAT'">
           ...
           <div v-if="item == 'WALLET'">
              ...
              <span class="method_name">{{$t('yezf')}}({{$t('dangqianyue')}}：Kz{{ walletValue | unitPrice }})</span>
            </div>
<!-- 			<div v-if="item == 'WALLET'">
			  <u-icon class="method_icon" name="red-packet-fill" color="#dd6161" size="80"></u-icon>
			  <span class="method_name"><view>{{$t('xianjinzhifu')}}</view></span>
			</div> -->
          </div>
