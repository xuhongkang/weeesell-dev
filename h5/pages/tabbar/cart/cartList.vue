<template>
  <div class="wrapper">
    <u-navbar :is-back="false" title="Cart">
      <div slot="right">
        <div class="light-color edit" @click="isEdit = !isEdit">{{ !isEdit ? $t('bj') : $t('wc')}}</div>
      </div>
    </u-navbar> 
    <!-- 空白页-->
    <view v-if="!loading && (cartDetail.cartList == '' || cartDetail.cartList == [] || !cartDetail)" class="empty">
      ...
      <view class="empty-tips">
		...
		{{$t('kkry')}}
		<!-- 随便逛逛 -->
        <navigator class="navigator" url="/pages/tabbar/home/index" open-type="switchTab">{{$t('sbgg')}}></navigator>
      </view>
    </view>
    <!-- 店铺商品信息 -->
    <div class="content">
          ...
          <view class="goods-row" :class="{ invalid: isInvalid(skuItem) }">
            <view class="goods-config">
              <view>
                ...
                <span class="invalid" v-else style="font-size: 24rpx">{{$t('sx')}}</span>
              </view>
            ...
            <view class="goods-content">
              ...
              <p class="sp-type" v-if="skuItem.goodsSku.salesModel == 'WHOLESALE'">{{$t('pfsp')}}</p>
              <p class="sp-number">
                <view class="sp-price">
                  <!-- <div class="default-color" :class="{'main-color':Object.keys(skuItem.promotionMap).length ==0  }"> -->
                  <div class="main-color">
                    Kz<span>{{ $options.filters.goodsFormatPrice(skuItem.goodsSku.price)[0] }}</span>
                ...
                >
                  <div class="promotions-item-seckill" v-if="getPromotion(skuItem).includes('SECKILL')">
                    {{$t('jlms')}}: <u-count-down show-border :hide-zero-day="true" :color="$mainColor" border-color="#ededed"
                ...
                <!-- <div class="priceDetail-flowPrice" :class="{'main-color':skuItem.priceDetailDTO}"
                  ...
                  预估到手价 Kz<span>{{ $options.filters.goodsFormatPrice(skuItem.priceDetailDTO.flowPrice)[0]}}</span>
    ...
    <div class="box box6">
      <view class="navL">
        <u-checkbox shape="circle" :active-color="lightColor" v-model="checkout" @change="checkOut()" label-size="24">{{$t('qx')}}
        </u-checkbox>
        <span class="price">
          <div class="prices">
            <div class="fullPrice">
              <span class="number" v-if="cartDetail && cartDetail.priceDetailDTO">
                {{$t('zj')}}:
                <span>Kz{{ $options.filters.goodsFormatPrice(cartDetail.priceDetailDTO.flowPrice)[0] }}</span>.<span>{{ $options.filters.goodsFormatPrice(cartDetail.priceDetailDTO.flowPrice)[1] }}</span>
              </span>
              <span class="number" v-else>{{$t('zj')}}:0.00</span>
            </div>
            <div
              ...
              class="discountPrice">
              <span>{{$t('yhj')}}{{(cartDetail.priceDetailDTO.goodsPrice - cartDetail.priceDetailDTO.flowPrice) | unitPrice}}
              </span>
              <span class="discount-details" @click="discountDetails">{{$t('yhmx')}}</span>
            </div>
          </div>
        </span>
      </view>
      ...
      <u-popup z-index="3" close mode="bottom" height="50%" closeable v-model="discountDetailsFlag" border-radius="20">
        <div class="discount-list">
          <view class="discount-title">{{$t('yhmx')}}</view>
          <div class="discount-way">
            <div class="discount-item" v-if="cartDetail.priceDetailDTO">
              <span>{{$t('spze')}}</span>
              <span>{{$t('zbfh')}}{{cartDetail.priceDetailDTO.goodsPrice | unitPrice}}</span>

            </div>
            <div class="discount-item" v-if="cartDetail.priceDetailDTO">
              <span>{{$t('yhj')}}</span>
              <span>-Kz{{cartDetail.priceDetailDTO.couponPrice | unitPrice}}</span>
            </div>
            <div class="discount-item" v-if="cartDetail.priceDetailDTO">
              <span>{{$t('qtyh')}}</span>
              <span>-{$t('zbfh')}}{{cartDetail.priceDetailDTO.discountPrice | unitPrice}}</span>
            </div>
          </div>
        </div>
      </u-popup>
      ...
      <view v-if="isEdit" @click="deleteGoods()">
        <div class="settlement">{{$t('sc')}}</div>
      </view>

      <view v-else @click="submitOrder()">
        <div class="settlement">{{$t('qjs')}}</div>
      </view>
    </div>
    <u-toast ref="uToast" />
  </div>
