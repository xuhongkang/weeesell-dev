		...
		<div v-if="!wechatLogin">
			...
			<div>
				...
				<div :class="current == 1 ? 'desc-light' : 'desc'">
					{{ loginTitleWay[current].desc}}<span v-if="current == 1">{{ mobile | secrecyMobile }}</span>
				</div>
			</div>
			<!-- 手机号 -->
			<div v-show="!enableUserPwdBox">
				<div v-show="current == 0">
					<u-input :custom-style="inputStyle" :placeholder-style="placeholderStyle" placeholder="Please enter your phone number (9 digits)"
						class="mobile" focus v-model="mobile" type="number" maxlength="9" />
				...
				<div v-show="current == 1" class="box-code">
					...
					<div class="fetch-btn">
						<u-verification-code change-text="Verification code has been sent（x）" end-text="Retrieve verification code again" unique-key="page-login"
			...
			<!-- 帐号密码登录 -->
			<div v-show="enableUserPwdBox">
				<u-input :custom-style="inputStyle" :placeholder-style="placeholderStyle" placeholder="Telephone"
					class="mobile" focus v-model="userData.username" />
				<u-input :custom-style="inputStyle" :placeholder-style="placeholderStyle" placeholder="Senha"
			...
			<div class="flex" v-show="current != 1">
				...
				<div class="tips">
					{{$t('weizhucedeshoujihaoma')}}<span @click="navigateToPrivacy('PRIVACY_POLICY')"><view>{{$t('yinsixieyi')}}</view></span>
					<span @click="navigateToPrivacy('USER_AGREEMENT')">
						<view>{{$t('yonghuxieyi')}}</view>
					</span>
				</div>
			</div>

			<div v-if="current != 1" class="user-password-tips" @click="enableUserPwdBox = !enableUserPwdBox">
				{{ !enableUserPwdBox ? $t('zhanghaomima') : $t('sjh') }}Login
			</div>
		...
		data() {
			return {
				...
				loginTitleWay: [{
					...
					{
						title: this.$t('qsryzm'),
						desc: this.$t('yanzhengmayifasongzhi'),
					},
				],
				// loginTitleWay: [{
				// 		title: "%hydl%",
		...
		methods: {
			...
			passwordLogin() {
				if (!this.enablePrivacy) {
				  ...
				  if (lang == 'en') {
					uni.showToast({
						title: "Please agree to user privacy",
						duration: 2000,
						icon: "none",
					});
				  }
					...
					uni.showToast({
						title: "Por favor, concorde com a privacidade do usuário",
						duration: 2000,
						icon: "none",
					});
				  }
