<template>
	<view class="content">
		...
		<view class="search-keyword" v-if="!isShowSeachGoods">
			...
			<div class="keyword-box" v-show="!isShowKeywordList">
				<view class="keyword-block add1">
					<view class="keyword-list-header"><view class="u-tips">{{$t('remensousuo')}}</view></view>
				...
				<view class="keyword-block" v-if="oldKeywordList.length > 0">
					<view class="keyword-list-header"><view class="u-tips">{{$t('sousuolishi')}}</view></view>
					<div class="oldKeyList">
						...
						<div @click="showMore" v-if="oldKeywordIndex > loadIndex" class="oldKeyItem">{{$t('zhanshigengduo')}}</div>
					</div>
				</view>

				<div class="clear mp-iphonex-bottom" @tap="oldDelete">{{$t('qingkongsousuolishi')}}</div>
			</div>
		</view>
		<!-- 搜索 -->
		<view class="goods-content" v-if="isShowSeachGoods">
			<view class="navbar">
				<view class="nav-item" :class="{ current: filterIndex === 0 }" @click="tabClick(0)">{{$t('zonghepaiming')}}</view>
				<view class="nav-item" :class="{ current: filterIndex === 3 }" @click="tabClick(3, 'buyCount')">
					<text>{{$t('xiaoliang')}}</text>
					<view class="p-box">
				...
				<view class="nav-item" @click="sortGoods">{{$t('shaixuan')}}</view>
			</view>
			<!-- 一行一个商品展示 -->
			<div v-if="isSWitch">
				<scroll-view
			...
			<view class="sort-box  ">
				<view class="sort-list">
					<view class="sort-item">
						<view class="sort-title">{{$t('pinpai')}}</view>
					...
					<view class="sort-item">
						<view class="sort-title">{{$t('quanbufenlei')}}</view>
				...
				<view class="sort-list">
					<view class="sort-item">
						<view class="sort-title">{{$t('jiagequyu')}}</view>
						<view style="display:flex;  margin-top:20rpx;    align-items: center;">
							<view class="sort-brand-item uinput">
								<view class="sort-radius"><u-input v-model="minPrice" type="number" placeholder="lowest price" input-align="center" /></view>
							</view>
							<view>-</view>
							<view class="sort-brand-item uinput">
								<view class="sort-radius"><u-input v-model="maxPrice" type="number" placeholder="Highest price" input-align="center" /></view>
							</view>
						</view>
					</view>
				</view>
				...
				<view class="sort-btn mp-iphonex-bottom">
					<view class="sort-btn-repick" @click="repick">{{$t('chongzhi')}}</view>
					<view class="sort-btn-confim" @click="sortConfim">{{$t('qd')}}</view>
				</view>
			</view>
		</u-popup>
		<div class="empty" v-if="empty">
			...
			<view>
				<p>{{$t('meizhaodaoxiangguanxiangpin')}}</p>
				<p>{{$t('qinghuanyigeshishi')}}</p>
			</view>
		</div>
...
export default {
	...
	watch: {
		...
		keyword(val) {
			if (val) {
				...
				this.defaultKeyword = $t('qingshurusousuoshangpin');
			}
		},
		sortPopup(val) {
			if (val) {
	...
	methods: {
		...
		doSearch(keyword) {
			...
			this.defaultKeyword == $t('qingshurusousuoshangpin') ? (keyword = '') : '';
