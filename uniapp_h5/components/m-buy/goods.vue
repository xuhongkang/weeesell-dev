			...
			<view class="goods-box bottom">
				...
				<scroll-view class="goods-skus-box" :scroll-y="true">
					...
					<view class="goods-skus-number flex flex-a-c flex-j-sb">
						<view class="view-class-title">{{$t('shuliang')}}</view>
						<uni-number-box class="uNumber" :min="1" :max="999"   v-model="num"></uni-number-box>
					</view>
				</scroll-view>
				<!-- 按钮 -->
				<view class="btns">
					<view class="box-btn card" v-if="buyType != 'PINTUAN' && goodsDetail.goodsType != 'VIRTUAL_GOODS'" @click="addToCartOrBuy('cart')">{{$t('jiarugouwuche')}}</view>
					<view class="box-btn buy" @click="addToCartOrBuy('buy')">{{$t('lijigoumai')}}</view>	
				</view>
			</view>
		</u-popup>
	</div>
