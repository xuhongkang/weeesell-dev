<template>
  <view class="address">
    <u-empty class="empty" v-if="this.addressList.length === 0" text="There is currently no shipping address available" mode="address"></u-empty>
    <view class="list" >
      <view class="item c-content" v-for="(item, index) in addressList" :key="index">
        ...
        <view class="edit">
          <view class="relative" @click="setDefault(item)">
            ...
            <text>{{ item.isDefault ? $t('morendizhi') :  $t('sheweimoren') }}</text>
          </view>
          <view class="relative">
            <view class="alifont icon-bianji-copy"></view>
            <text class="mr-40" @click="addAddress(item.id)">{{$t('bj')}}</text>
            <view class="alifont icon-lajitong"></view>
            <text @click="removeAddress(item.id)">{{$t('sc')}}</text>
          </view>
        </view>
      </view>
