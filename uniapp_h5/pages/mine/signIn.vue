      ...
      <div class="box">
        <div class="circle-box">
          <div class="cricle" @click="signIn()">
            <span v-if="!ifSign" :class="{ active: signFlag || ifSign }"><view>{{$t('qiandao')}}</view></span>
            <span v-else :class="{ active: signFlag || ifSign }"
              :style="ifSign ? 'transform: rotateY(0deg);' : ''"><view>{{$t('yiqian')}}</view></span>
          </div>
        </div>
        <text class="tips">{{$t('meitianjianchiqiandao')}}</text>
      </div>
    </view>
    <div class="date-card">
      <view class="date-con">
        <view class="date-tit">
          <div class="current-month">
            <div class="day">{{$t('meirijil')}}<span> ({{ currentMonth }})</span></div>
          </div>
        </view>
        <view class="week">
          <text v-for="item in weekArr" :key="item.id">{{$t(item)}}</text>
        </view>
