    ...
    <view class="store-info">
      ...
      <view class="name-star star-con">
        <div class="name">
          ...
          <span v-if="storeDetail.selfOperated == 1" class="shopTag"><view>{{$t('ziying')}}</view></span>
        </div>
        <div class="store-row">
          <div class="collectionNum">{{ storeDetail.collectionNum || 0 }}{{$t('guanzhu')}}</div>
          <div class="goodsNum">{{ storeDetail.goodsNum || 0 }}{{$t('jianshangpin')}}</div>
        </div>
      </view>
      <view class="to-store-btn" @click="tostorePage(goodsDetail)">
        <view>{{$t('jindianguangguang')}}</view>
      </view>
    </view>

    <view class="store-recommend">
      <view class="store-recommend-title">{{$t('dianneiqitashangpin')}}</view>
      <view class="recommend-list">
        <view class="recommend-item" @click="clickGoods(item)" v-for="(item, index) in res" :key="index">
          <u-image class="recommend-item-img" :fade="true" duration="450" :lazy-load="true" :src="item.thumbnail" height="218rpx">
            ...
            <view slot="error" style="font-size: 24rpx; ">{{$t('jiazaishibai')}}</view>
          </u-image>
          <view class="recommend-item-name">
            {{ item.goodsName }}
          </view>
          <view class="item-price" v-if="item.price != undefined">
            Kz<span class="item-price-blod">{{ $options.filters.goodsFormatPrice(item.price)[0] }}</span>.{{ $options.filters.goodsFormatPrice(item.price)[1] }}
          </view>
        </view>
      </view>
    </view>
