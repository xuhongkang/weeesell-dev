      ...
      <swiper class="swiper-box" :current="current" @change="changeSwiper" duration="500">
        <swiper-item v-for="(item, listIndex) in list" :key="listIndex">
          <scroll-view scroll-y style="height: 100%" @scrolltolower="renderData(listIndex)">
            <u-empty text="no data" mode="list" v-if="orderList.length == 0"></u-empty>
                ...
                <view v-if="current == 1 && sku.commentStatus == 'UNFINISHED'">
                  <view class="evaluate">
                    <view @click="talkCommont(sku)">
                      <u-tag text="Publish comments" shape="circle" mode="plain" type="error" />
                    </view>
                  </view>
                </view>
  ...
  data() {
    return {
      ...
      gradeList: {
        //评论表
        GOOD: this.$t('haoping'),
        MODERATE: this.$t('zhongping'),
        WORSE: this.$t('chaping'),
        haveImage: this.$t('youtu'),
      },
      groupCommentStatusWay: {
        NEW: this.$t('NEW'),
        UNFINISHED: this.$t('UNFINISHED'),
        WAIT_CHASE: this.$t('WAIT_CHASE'),
