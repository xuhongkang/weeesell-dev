...
<!--    <div class="u-tabs-search">
      <u-search
        placeholder="Entrar o número do pedido"
    ...
    <scroll-view class="body-view" scroll-y @scrolltolower="renderDate">
      ...
      >
        ...
        <view class="seller-info u-flex u-row-between" v-if="current == 0">
          ...
          <view class="order-sn">{{$t('ddbh')}}:{{ order.sn }}</view>
        </view>
        <!-- 申请记录 选项卡 -->
        <view class="seller-info u-flex u-row-between" v-if="current != 0">
          <view class="order-sn"
            >{{$t('shouhoudanhao')}}：{{ order.service_sn || order.sn }}</view
          >
        ...
        <view v-for="(sku, goodsIndex) in order.orderItems" :key="goodsIndex">
          ...
          <view class="description">
            ...
            <view v-if="current === 0 && sku.afterSaleStatus">
              ...
              >
                ...
                {{$t('gaishangpinyishenqingshouhou')}}
              </view>
            </view>
            <view v-if="current === 0 && sku.afterSaleStatus">
              <view
            ...
            <div v-if="current === 1 || current === 2">
              ...
              >
                {{$t('tuihuochuli')}}-{{ order.serviceStatus | serviceStatusList }}</view
              >
              <view
                class="cannot_apply not_center"
                v-if="order.serviceType == 'SUPPLY_AGAIN_GOODS'"
              >
                {{$t('bufasahngpin')}}-{{ order.serviceStatus | serviceStatusList }}</view
              >
              <view
                class="cannot_apply not_center"
                v-if="order.serviceType == 'RETURN_MONEY'"
              >
                {{$t('tuikuan')}}-{{ order.serviceStatus | serviceStatusList }}</view
              >
              <view
                class="cannot_apply not_center"
                v-if="order.serviceType == 'EXCHANGE_GOODS'"
              >
                {{$t('huanhuo')}}-{{ order.serviceStatus | serviceStatusList }}</view
              >
              <view
                class="cannot_apply not_center"
                v-if="order.serviceType == 'CANCEL'"
              >
                {{$t('qxdd')}}-{{ order.serviceStatus | serviceStatusList }}</view
              >
            </div>
          ...
          <view class="btn-view u-flex u-row-right">
            ...
            <div class="sale" v-if="current === 0 && sku.afterSaleStatus">
              ...
              >
                <view class="default-btn border"> {{$t('shouhoushenqing')}} </view>
              </div>
            </div>
        ...
        >
          ...
          <view class="cannot_apply">
            {{$t('ddzje')}}:<span class="countMoney"
              >Kz{{ order.flowPrice | unitPrice }}</span
            >
          </view>
        </view>
      </view>
  ...
  data() {
    return {
      list: [
        //tab表头
        {
          name: this.$t('shouhoushenqing'),
        },
        {
          name: this.$t('shenqingzhong'),
        },
        {
          name: this.$t('shenqingjilu'),
        },
      ],
      current: 0, //当前表头索引
      tipsShow: false, //提示开关
